(function() {
  ...
  Generator = (function() {
    ...
    Generator.prototype.render = function(callback) {
      var defaultTemplateFiles, localTempalateFiles;
      var _this = this;
      defaultTemplateFiles = this.sourceFiles.map(function(file) {
        return path.join(__dirname, '..', file);
      });
      localTempalateFiles = this.sourceFiles.map(function(file) {
        return path.join(this.projectDirectory, file);
      });
      return async.map(localTempalateFiles, (function(file, callback) {
        ...
        templateFiles = (function() {
          ...
          for (i = 0, _len = results.length; i < _len; i++) {
            result = results[i];
            if (result) {
              _results.push(localTempalateFiles[i]);
            } else {

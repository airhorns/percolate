...
exports['MultiConsole transformation'] = Test.Case
  'should transform several consoles into one multi console when having not just consoles': (test) -> 
    ...
    (new Percolate.MultiConsoleTransformation).walk(root)
  ...
  'should transform several consoles into one multi console when having not just consoles': (test) -> 
    ...
    test.equals root.children[3].children.length, 3
    test.done()

  'should transform several consoles into one multi console when only having consoles': (test) -> 
    root = Test.getTestCase ->
      @equal true, -> true
      @equal 1, -> true
      @notEqual true, -> false
  ...
  'should transform several consoles into one multi console when only having assertions': (test) -> 
    root = Test.getTestCase ->
      @show -> false
      @show -> true
      @show -> 'bar'
  ...
  'should transform several consoles into one multi console in nested cases': (test) -> 
    root = Test.getTestCase ->
      @explain 'something'
      @test 'nestedTest', ->
        @explain 'start'
